...
def evaluate_encoder(model, tr_loader, te_loader, device, seed, tag):
    """Extract latents and run fresh probes for every attribute."""
    X_tr, y_tr = extract(model, tr_loader, device)
    X_te, y_te = extract(model, te_loader, device)

    print(f"\n  {tag}: fresh probes on frozen latents")
    res = {}
    for attr, n_cls in ATTRS.items():
...
def main():
    ...
    print(f"Device: {device}  |  lambda_max {args.lambda_max}\n")

    tf = transforms.Compose([transforms.Resize((96, 96)), transforms.ToTensor()])
    ds = UTKFaceDataset(args.data_root, transform=tf)

    # reuse the exact split from extract_latents so results are comparable
    ref = np.load('../../latents/utkface_latents.npz', allow_pickle=True)
    split = ref['split']
    ...
    print("=" * 70)
    results['before'], _ = evaluate_encoder(model, tr_eval, te_eval,
                                            device, args.seed, 'baseline')
    ...
    results['after'], (Xtr, ytr, Xte, yte) = evaluate_encoder(
        model, tr_eval, te_eval, device, args.seed, 'debiased')

    # --- verdict ---
    print("\n" + "=" * 70)
    print(f"{'attribute':<13}{'chance':>9}{'before':>10}{'after':>10}"
